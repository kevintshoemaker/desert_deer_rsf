...
### run basic analysis without random forest, using glmmTMB
  # make figure for model coefficients
  # make partial dependence plots 
  # figure for survival analysis
  # write up new methods
  # ** should we include a year effect (random intercept)? 
...
VisualizeRelation <- function(data=df_rsf2,model=mod_final2,predvar="NDVI",varname="NDVI",allvars=predvars,status="atheel"){
  ...
  tmp <- table(data$ID,data$STATUS)[,status]
  IDs <- as.numeric(names(tmp)[tmp>100])
  ...
  for(id in 1:min(30,length(IDs))){
    newdata$ID <- IDs[id]  #unique(data$ID)[id]
    pred2[,id] <- predict(model,newdata,type="conditional",se.fit=F)
    #lines(range,pred2[,id]*1000,col=cols[id],lwd=1)
  }
  
  plot(range,pred*1000,xlab=varname,ylab="Sel. Intensity",type="l",lwd=2,xaxt="n",col="black",ylim=c(min(pred2)*1000,min(2,max(pred2)*1000)))
...
df_rsf <- read.csv("all_repro_data_with_dates_clean.csv",stringsAsFactors = F)  #     All_repro_use_avail_data_ch2.csv
df_rsf$STATUS <- as.factor(df_rsf$STATUS)
...
temp <- lapply(allvars,function(t) {df_rsf[[t]] <<- scale(df_rsf[[t]])}   )  # standardize

lapply(allvars,function(t) hist(df_rsf[[t]],main=t))
...
predvars <- c("Rugged","dist2WATER","NDVI","elevation")   # remove ruggedness or slope
...
df_rsf$ID2 <- as.factor(df_rsf$ID_YR)

df_rsf$weights <- ifelse(df_rsf$USED == 1, 1, 1000)

temp <- table(df_rsf$ID_YR,df_rsf$STATUS)
full_inds <- names(apply(temp,1,function(t) all(t>200) ))[apply(temp,1,function(t) all(t>200) )]
# 20 ids with all three phases


df_rsf3 <- df_rsf[df_rsf$ID_YR%in%full_inds,]
nrow(df_rsf3)
df_rsf4 <- df_rsf3
df_rsf4 <- df_rsf4[rep(FALSE,times=nrow(df_rsf4)),]
## thin the dataset for model testing
id="414_2014"
...
mod_comp2.3 <- glmmTMB(USED ~ NDVI + elevation + Rugged + dist2WATER #+ STATUS +   # this one works   this is the best model
                         NDVI:STATUS + elevation:STATUS + Rugged:STATUS + dist2WATER:STATUS + 
                         (1|ID) + #(0+STATUS|ID) +
                         (0 + NDVI | ID/STATUS) + (0 + elevation | ID/STATUS) + 
                         (0 + Rugged | ID/STATUS) + (0 + dist2WATER | ID/STATUS),
                       weights = weights,
                       family=binomial,
...
TMBStruc = glmmTMB(USED ~ NDVI + elevation + Rugged + dist2WATER #+ STATUS +   # this one works   this is the best model
                   NDVI:STATUS + elevation:STATUS + Rugged:STATUS + dist2WATER:STATUS + 
                     (1|ID) + #(0+STATUS|ID) +
                     (0 + NDVI | ID/STATUS) + (0 + elevation | ID/STATUS) + 
                     (0 + Rugged | ID/STATUS) + (0 + dist2WATER | ID/STATUS),
...
save(mod_final, mod_final2, file= "rsf_finalmodels.RData")

# load("rsf_finalmodels.RData")
...
cis <- confint(mod_final,component="cond",method="wald")   # takes a little while.    #,method="profile"
fes <- fixef(mod_final)$cond
table1 <- data.frame(
  label1 = names(fes)[c(1,2,6,7,3,8,9,4,10,11,5,12,13)],
  label2 = c("Intercept","NDVI","NDVI (post mort)","NDVI (pre part)","Elevation","Elevation (post mort)","Elevation (pre part)","Ruggedness","Ruggedness (post mort)","Ruggedness (pre part)","Distance to Water","Distance to Water (post mort)","Distance to Water (pre part)")
)
...
table1$sig <- ifelse(sign(sign(table1[3])*sign(table1[,4]))==1,"*","")
names(table1) <- c("","Coef","CI,lower","CI,upper","")
...
svg(filename="tradeoffs_v1.svg",width=7,height=4)

df <- subset(df_rsf,USED==0)

rugsel <- ranef(mod_final)$cond$ID[,"Rugged"]
names(rugsel) <-  rownames(ranef(mod_final)$cond$ID)

ndviav <- sapply(names(rugsel),function(t) mean(subset(df,ID==t)$NDVI)  )
rugav <- sapply(names(rugsel),function(t) mean(subset(df,ID==t)$Rugged)  )
...
svg(filename="tradeoffs_v2.svg",width=6,height=7)
...
for(r in rep_stats){
  ...
  allinds <- rownames(ranef(mod_final)$cond$ID)
  ndx2 <- grepl(r,rownames(ranef(mod_final)$cond$'STATUS:ID'))
  ndx3 <- grepl(sprintf("Rugged:STATUS%s",r),names(fixef(mod_final)$cond))
  thisinds <- gsub(sprintf("%s:",r),"", rownames(ranef(mod_final)$cond$'STATUS:ID'[ndx2,]))
  ndx4 <- sapply(allinds,function(t) t%in%thisinds  )
  
  rugsel <- ranef(mod_final)$cond$ID[ndx4,"Rugged"] + ranef(mod_final)$cond$'STATUS:ID'[ndx2,"Rugged"] + fixef(mod_final)$cond["Rugged"] + ifelse(r!="atheel", fixef(mod_final)$cond[ndx3],0)
  names(rugsel) <-  rownames(ranef(mod_final)$cond$ID)[ndx4]
...
for(repro in rep_stats){
  for(var in predvars){
    ...
    IDz <- unique(used$ID)
    meanz_u <- sapply(IDz,function(t) mean(used[[var]][used$ID==t]))
    meanz_a <- sapply(IDz,function(t) mean(avail[[var]][used$ID==t]))

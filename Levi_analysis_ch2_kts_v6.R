...
#################
# Code for Heffelfinger et al 2020: frontiers in E + E
...
## NOTE: files were separated by individual to enable upload of data to github

allfiles <- list.files()[grepl("allpoints_",list.files())]

df_rsf <- lapply(allfiles,read.csv)
df_rsf <- do.call("rbind", df_rsf)
...
mod_comp2.3 <- glmmTMB(USED ~ NDVI + elevation + rugged + water + STATUS +   # this is the best model
...
#########
# Make smaller datasets for sharing to github
...
# names(df_rsf)      # takes a while to read with 50:1 background to used ratio
# 
# allinds <- sort(unique(df_rsf$ID_YR))
# 
# temp <- sapply(allinds,function(t) write.csv(subset(df_rsf,ID_YR==t),file=sprintf("allpoints_%s.csv",t))   )
